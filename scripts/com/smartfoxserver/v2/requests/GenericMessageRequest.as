package com.smartfoxserver.v2.requests
{
   ...
   public class GenericMessageRequest extends BaseRequest
   {
      ...
      public static const KEY_AOI:String = "aoi";
      ...
      private function validatePublicMessage(param1:SmartFox, param2:Array) : void
      {
         ...
         if(this._room != null && param1.joinedRooms.indexOf(this._room) < 0)
         {
      ...
      private function executeObjectMessage(param1:SmartFox) : void
      {
         ...
         var _loc4_:ListSet = new ListSet();
         if(this._recipient is Array)
         {
            _loc2_ = this._recipient as Array;
            if(_loc2_.length > this._room.capacity)
            {
               throw new ArgumentError("The number of recipients is bigger than the target Room capacity: " + _loc2_.length);
            }
            for each(_loc3_ in _loc2_)
            {
               if(_loc3_ is User)
               {
                  _loc4_._SafeStr_1(_loc3_.id);
               }
               else
               {
                  param1.logger.warn("Bad recipient in ObjectMessage recipient list: " + typeof _loc3_ + ", expected type: User");
               }
            }
         }
         _sfso.putInt(KEY_ROOM_ID,this._room.id);
         ...
         if(_loc4_.size() > 0)
         {
            _sfso.putIntArray(KEY_RECIPIENT,_loc4_.toArray());
         }
      }
   }
}
