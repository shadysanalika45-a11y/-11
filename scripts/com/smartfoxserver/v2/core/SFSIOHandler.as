package com.smartfoxserver.v2.core
{
   ...
   public class SFSIOHandler implements IoHandler
   {
      ...
      public function SFSIOHandler(param1:BitSwarmClient)
      {
         ...
         this.log = param1.sfs.logger;
         this.packetEncrypter = new DefaultPacketEncrypter(param1);
      ...
      private function handleNewPacket(param1:ByteArray) : ByteArray
      {
         ...
         return this.resizeByteArray(param1,1,0);
      }
      ...
      private function handleDataSizeFragment(param1:ByteArray) : ByteArray
      {
         ...
         var _loc3_:int = this.pendingPacket.header.bigSized ? int(4 - this.pendingPacket.buffer.position) : int(2 - this.pendingPacket.buffer.position);
         if(param1.length >= _loc3_)
         {
            this.pendingPacket.buffer.writeBytes(param1,0,_loc3_);
            this.pendingPacket.buffer.position = 0;
            _loc2_ = this.pendingPacket.header.bigSized ? int(this.pendingPacket.buffer.readInt()) : int(this.pendingPacket.buffer.readShort());
            this.log.debug("DataSize is ready:",_loc2_,"bytes");
            this.pendingPacket.header.expectedLen = _loc2_;
      ...
      private function handlePacketData(param1:ByteArray) : ByteArray
      {
         ...
         if(param1.length >= _loc2_)
         {
            ...
            if(this.pendingPacket.header.encrypted)
            {
               this.packetEncrypter.decrypt(this.pendingPacket.buffer);
            }
      ...
      public function onDataWrite(param1:IMessage) : void
      {
         ...
         if(this.bitSwarm.cryptoKey != null && !this.bitSwarm.isReconnecting)
         {
            this.packetEncrypter.encrypt(_loc3_);
            _loc5_ = true;
         }
         var _loc6_:int = SHORT_BYTE_SIZE;
         if(_loc3_.length > 65535)
         {
            _loc6_ = INT_BYTE_SIZE;
         }
         var _loc7_:PacketHeader = new PacketHeader(_loc5_,_loc4_,false,_loc6_ == INT_BYTE_SIZE);
         _loc2_.writeByte(_loc7_.encode());
         if(_loc6_ > SHORT_BYTE_SIZE)
         {
         ...
         if(this.bitSwarm.useWebSocket)
         {
            this.bitSwarm.webSocket.send(_loc2_);
         }
         else if(this.bitSwarm.useBlueBox)
         {
