package com.smartfoxserver.v2
{
   ...
   public class SmartFox extends EventDispatcher
   {
      ...
      private var _minVersion:int = 7;
      
      private var _subVersion:int = 9;
      
      public var _bitSwarm:BitSwarmClient;
      ...
      public function setClientDetails(param1:String, param2:String) : void
      {
         if(this._isConnected)
         {
            this.logger.warn("SetClientDetails must be called before the connection is started");
            return;
         }
         this._clientDetails = param1 != null ? param1.replace(this.CLIENT_TYPE_SEPARATOR," ") : "";
         this._clientDetails += this.CLIENT_TYPE_SEPARATOR;
         this._clientDetails += param2 != null ? param2.replace(this.CLIENT_TYPE_SEPARATOR," ") : "";
      }
      ...
      public function get socketEngine() : BitSwarmClient
      {
      ...
      public function get httpUploadURI() : String
      {
         ...
         return "http://" + this.config.host + ":" + this.config.httpPort + "/BlueBox/SFS2XFileUpload?sessHashId=" + this.sessionToken;
      }
      ...
      public function connectWithConfig(param1:ConfigData) : void
      {
         this.validateAndStoreConfig(param1);
         this.connect();
      }
      ...
      private function onConfigLoadSuccess(param1:SFSEvent) : void
      {
         ...
         this.validateAndStoreConfig(_loc3_);
      ...
      private function handleHandShake(param1:SFSEvent) : void
      {
         ...
         var _loc5_:IMessage = param1.params.message;
         var _loc6_:ISFSObject = _loc5_.content;
         if(_loc6_.isNull(BaseRequest.KEY_ERROR_CODE))
         {
            this._sessionToken = _loc6_.getUtfString(HandshakeRequest.KEY_SESSION_TOKEN);
            this._bitSwarm.compressionThreshold = _loc6_.getInt(HandshakeRequest.KEY_COMPRESSION_THRESHOLD);
            this._bitSwarm.maxMessageSize = _loc6_.getInt(HandshakeRequest.KEY_MAX_MESSAGE_SIZE);
         ...
         {
            _loc2_ = _loc6_.getShort(BaseRequest.KEY_ERROR_CODE);
            _loc3_ = SFSErrorCodes.getErrorMessage(_loc2_,_loc6_.getUtfStringArray(BaseRequest.KEY_ERROR_PARAMS));
            _loc4_ = {
               "success":false,
               "errorMessage":_loc3_,
               "errorCode":_loc2_
            };
            dispatchEvent(new SFSEvent(SFSEvent.CONNECTION,_loc4_));
         }
      }
      ...
      kernel function handleClientDisconnection(param1:String) : void
      {
      ...
      kernel function handleLogout() : void
      {
         ...
         this._currentZone = null;
         this._mySelf = null;
         this.buddyManager.clearAll();
         this.buddyManager.setInited(false);
      }
      
      kernel function handleReconnectionFailure() : void
      {
         this.setReconnectionSeconds(0);
         this._bitSwarm.stopReconnection();
      }
      ...
      private function handleConnectionProblem(param1:BitSwarmEvent) : void
      {
         ...
         if(this._bitSwarm.connectionMode == ConnectionMode.SOCKET && this._useBlueBox)
         {
            ...
            _loc2_ = this.DEFAULT_HTTP_PORT;
            if(this.config != null)
            {
               _loc2_ = this.config.forceBlueBoxOverHttps ? int(this.config.httpsPort) : int(this.config.httpPort);
            }
      ...
      {
         var _loc2_:IRequest = new HandshakeRequest(this.version,this._clientDetails,param1 ? this._sessionToken : null);
         this.send(_loc2_);
      }
      
      private function validateAndStoreConfig(param1:ConfigData) : void
      {
         if(param1.host == null || param1.host.length == 0)
         {
            throw new ArgumentError("Invalid Host/IpAddress in external config file");
         }
         if(param1.port < 0 || param1.port > 65535)
         {
            throw new ArgumentError("Invalid TCP port in external config file");
         }
         if(param1.zone == null || param1.zone.length == 0)
         {
            throw new ArgumentError("Invalid Zone name in external config file");
         }
         this._debug = param1.debug;
         this._useBlueBox = param1.useBlueBox;
         this._config = param1;
      }
   }
}
