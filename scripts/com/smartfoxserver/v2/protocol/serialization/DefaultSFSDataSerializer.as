package com.smartfoxserver.v2.protocol.serialization
{
   ...
   public class DefaultSFSDataSerializer implements ISFSDataSerializer
   {
      ...
      private function obj2bin(param1:ISFSObject, param2:ByteArray) : ByteArray
      {
         ...
         {
            _loc3_ = param1.getData(_loc4_);
            param2 = this.encodeSFSObjectKey(param2,_loc4_);
      ...
      private function decodeObject(param1:ByteArray) : SFSDataWrapper
      {
         ...
         if(_loc6_ == SFSDataType.NULL)
         {
            _loc2_ = this.binDecode_NULL(param1);
         }
         else if(_loc6_ == SFSDataType.BOOL)
         {
            _loc2_ = this.binDecode_BOOL(param1);
         }
         else if(_loc6_ == SFSDataType.BOOL_ARRAY)
         {
            _loc2_ = this.binDecode_BOOL_ARRAY(param1);
         }
         else if(_loc6_ == SFSDataType.BYTE)
         {
            _loc2_ = this.binDecode_BYTE(param1);
         }
         else if(_loc6_ == SFSDataType.BYTE_ARRAY)
         {
            _loc2_ = this.binDecode_BYTE_ARRAY(param1);
         }
         else if(_loc6_ == SFSDataType.SHORT)
         {
            _loc2_ = this.binDecode_SHORT(param1);
         }
         else if(_loc6_ == SFSDataType.SHORT_ARRAY)
         {
            _loc2_ = this.binDecode_SHORT_ARRAY(param1);
         }
         else if(_loc6_ == SFSDataType.INT)
         {
            _loc2_ = this.binDecode_INT(param1);
         }
         else if(_loc6_ == SFSDataType.INT_ARRAY)
         {
            _loc2_ = this.binDecode_INT_ARRAY(param1);
         }
         else if(_loc6_ == SFSDataType.LONG)
         {
            _loc2_ = this.binDecode_LONG(param1);
         }
         else if(_loc6_ == SFSDataType.LONG_ARRAY)
         {
            _loc2_ = this.binDecode_LONG_ARRAY(param1);
         }
         else if(_loc6_ == SFSDataType.FLOAT)
         {
            _loc2_ = this.binDecode_FLOAT(param1);
         }
         else if(_loc6_ == SFSDataType.FLOAT_ARRAY)
         {
            _loc2_ = this.binDecode_FLOAT_ARRAY(param1);
         }
         else if(_loc6_ == SFSDataType.DOUBLE)
         {
            _loc2_ = this.binDecode_DOUBLE(param1);
         }
         else if(_loc6_ == SFSDataType.DOUBLE_ARRAY)
         {
            _loc2_ = this.binDecode_DOUBLE_ARRAY(param1);
         }
         else if(_loc6_ == SFSDataType.UTF_STRING)
         {
            _loc2_ = this.binDecode_UTF_STRING(param1);
         }
         else if(_loc6_ == SFSDataType.TEXT)
         {
            _loc2_ = this.binDecode_TEXT(param1);
         }
         else if(_loc6_ == SFSDataType.UTF_STRING_ARRAY)
         {
            _loc2_ = this.binDecode_UTF_STRING_ARRAY(param1);
         }
         else if(_loc6_ == SFSDataType.SFS_ARRAY)
         {
            param1.position -= 1;
         ...
         {
            if(_loc6_ != SFSDataType.SFS_OBJECT)
            {
               throw new Error("Unknow SFSDataType ID: " + _loc6_);
            }
            param1.position -= 1;
            _loc3_ = this.decodeSFSObject(param1);
            _loc4_ = SFSDataType.SFS_OBJECT;
            _loc5_ = _loc3_;
            if(_loc3_.containsKey(CLASS_MARKER_KEY) && _loc3_.containsKey(CLASS_FIELDS_KEY))
            {
               _loc4_ = SFSDataType.CLASS;
               _loc5_ = this.sfs2as(_loc3_);
            }
            _loc2_ = new SFSDataWrapper(_loc4_,_loc5_);
         }
         return _loc2_;
      }
      ...
      private function encodeObject(param1:ByteArray, param2:int, param3:*) : ByteArray
      {
         switch(param2)
         {
            ...
            case SFSDataType.TEXT:
               param1 = this.binEncode_TEXT(param1,param3 as String);
               break;
      ...
      private function encodeLongValue(param1:Number, param2:ByteArray) : void
      {
         ...
         if(param1 > -1)
         {
            _loc5_ = param1 / Math.pow(2,32);
            _loc6_ = param1 % Math.pow(2,32);
         }
         else
         {
            _loc3_ = Math.abs(param1);
            _loc4_ = _loc3_ - 1;
            _loc5_ = _loc4_ / Math.pow(2,32);
            _loc6_ = _loc4_ % Math.pow(2,32);
            _loc5_ = ~_loc5_;
            _loc6_ = ~_loc6_;
         }
      ...
      private function binDecode_TEXT(param1:ByteArray) : SFSDataWrapper
      {
         var _loc2_:int = param1.readInt();
         var _loc3_:String = param1.readUTFBytes(_loc2_);
         return new SFSDataWrapper(SFSDataType.TEXT,_loc3_);
      }
      ...
      private function binEncode_TEXT(param1:ByteArray, param2:String) : ByteArray
      {
         var _loc3_:ByteArray = new ByteArray();
         _loc3_.writeByte(SFSDataType.TEXT);
      ...
      private function convertAsObj(param1:*, param2:ISFSObject) : void
      {
         ...
         var _loc7_:as3reflect.Type = as3reflect.Type.forInstance(param1);
         var _loc8_:String = this.encodeClassName(ClassUtils.getFullyQualifiedName(_loc7_.clazz));
         if(_loc8_ == null)
         {
         ...
         {
            throw new SFSCodecError("Cannot serialize object: " + param1 + ", type: " + _loc8_ + " -- It doesn\'t implement the SerializableSFSType interface");
         }
         var _loc9_:ISFSArray = SFSArray.newInstance();
         param2.putUtfString(CLASS_MARKER_KEY,_loc8_);
         param2.putSFSArray(CLASS_FIELDS_KEY,_loc9_);
         for each(_loc3_ in _loc7_.fields)
         {
            if(!_loc3_.isStatic)
            {
               _loc4_ = _loc3_.name;
               ...
               if(_loc4_.charAt(0) != "$")
               {
                  _loc6_ = SFSObject.newInstance();
                  _loc6_.putUtfString(FIELD_NAME_KEY,_loc4_);
                  _loc6_.put(FIELD_VALUE_KEY,this.wrapASField(_loc5_));
                  _loc9_.addSFSObject(_loc6_);
               }
            }
         }
      }
      ...
      private function wrapASField(param1:*) : SFSDataWrapper
      {
         ...
         var _loc3_:String = as3reflect.Type.forInstance(param1).name;
      ...
      {
         var _loc2_:* = null;
         var _loc3_:ISFSObject = SFSObject.newInstance();
         for(_loc2_ in param1)
         {
            _loc3_.put(_loc2_,this.wrapASField(param1[_loc2_]));
         }
         return _loc3_;
      }
      ...
      public function sfs2as(param1:ISFSObject) : *
      {
         ...
         var _loc4_:Class = ClassUtils.forName(_loc3_);
         _loc2_ = new _loc4_();
      ...
      private function rebuildDict(param1:ISFSObject) : Object
      {
         ...
         {
            _loc3_[_loc2_] = this.unwrapAsField(param1.getData(_loc2_));
         }
         return _loc3_;
      }
      ...
      private function _scanSFSObject(param1:ISFSObject, param2:Object) : void
      {
         ...
         {
            _loc4_ = param1.getData(_loc3_);
            if(_loc4_.type == SFSDataType.NULL)
            {
               param2[_loc3_] = null;
            }
            else if(_loc4_.type == SFSDataType.SFS_OBJECT)
            {
               ...
               this._scanSFSObject(_loc4_.data as ISFSObject,_loc5_);
            }
            else if(_loc4_.type == SFSDataType.SFS_ARRAY)
            {
               param2[_loc3_] = (_loc4_.data as SFSArray).toArray();
            }
      ...
      private function _scanGenericArray(param1:Array, param2:ISFSArray, param3:Boolean = false) : void
      {
         ...
         while(_loc6_ < param1.length)
         {
            ...
            else if(_loc4_.toString() == "[object Object]" && !(_loc4_ is Array))
            {
               param2.addSFSObject(this.genericObjectToSFSObject(_loc4_,param3));
            }
            else if(_loc4_ is Array)
            {
               _loc5_ = new SFSArray();
               param2.addSFSArray(_loc5_);
               this._scanGenericArray(_loc4_,_loc5_,param3);
            }
            else if(_loc4_ is Boolean)
            {
               param2.addBool(_loc4_);
            }
            else if(_loc4_ is int && !param3)
            {
               param2.addInt(_loc4_);
            }
            else if(_loc4_ is Number)
            {
               param2.addDouble(_loc4_);
            }
            else if(_loc4_ is String)
            {
               param2.addUtfString(_loc4_);
            }
            _loc6_++;
         }
      }
      ...
      private function _scanSFSArray(param1:ISFSArray, param2:Array) : void
      {
         ...
         {
            _loc3_ = param1.getWrappedElementAt(_loc5_);
            if(_loc3_.type == SFSDataType.NULL)
            {
               param2[_loc5_] = null;
            }
            else if(_loc3_.type == SFSDataType.SFS_OBJECT)
            {
               param2[_loc5_] = (_loc3_.data as SFSObject).toObject();
            }
            else if(_loc3_.type == SFSDataType.SFS_ARRAY)
            {
               _loc4_ = [];
               param2[_loc5_] = _loc4_;
               this._scanSFSArray(_loc3_.data as ISFSArray,_loc4_);
            }
            else if(_loc3_.type != SFSDataType.CLASS)
            {
               param2[_loc5_] = _loc3_.data;
            }
            _loc5_++;
         }
      }
