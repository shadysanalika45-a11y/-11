package com.smartfoxserver.v2.entities.data
{
   ...
   public class SFSObject implements ISFSObject
   {
      ...
      private function dump() : String
      {
         ...
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc4_:String = DefaultObjectDumpFormatter.TOKEN_INDENT_OPEN;
         for(_loc3_ in this.dataHolder)
         {
            _loc1_ = this.getData(_loc3_);
            _loc2_ = _loc1_.type;
            _loc4_ += "(" + SFSDataType.fromId(_loc1_.type).toLowerCase() + ")";
            _loc4_ = _loc4_ + (" " + _loc3_ + ": ");
            if(_loc2_ == SFSDataType.SFS_OBJECT)
            {
               _loc4_ += (_loc1_.data as SFSObject).getDump(false);
            }
            else if(_loc2_ == SFSDataType.SFS_ARRAY)
            {
               _loc4_ += (_loc1_.data as SFSArray).getDump(false);
            }
            else if(_loc2_ == SFSDataType.BYTE_ARRAY)
            {
               _loc4_ += DefaultObjectDumpFormatter.prettyPrintByteArray(_loc1_.data as ByteArray);
            }
            else if(_loc2_ > SFSDataType.UTF_STRING && _loc2_ < SFSDataType.CLASS)
            {
               _loc4_ += "[" + _loc1_.data + "]";
            }
            else
            ...
            _loc4_ += DefaultObjectDumpFormatter.TOKEN_DIVIDER;
         }
         if(this.size() > 0)
         {
            _loc4_ = _loc4_.slice(0,_loc4_.length - 1);
         }
         return _loc4_ + DefaultObjectDumpFormatter.TOKEN_INDENT_CLOSE;
      }
      ...
      public function putText(param1:String, param2:String) : void
      {
         this.dataHolder[param1] = new SFSDataWrapper(SFSDataType.TEXT,param2);
      }
