package com.oyunstudyosu.debug
{
   ...
   public class DebugConsole
   {
      ...
      private static var _saveBtn:Sprite;
      ...
      private static function buildSaveButton() : void
      {
         var tf:TextField;
         _saveBtn = new Sprite();
         _saveBtn.buttonMode = true;
         _saveBtn.mouseChildren = false;
         _saveBtn.graphics.beginFill(0,0.75);
         _saveBtn.graphics.drawRoundRect(0,0,140,26,10,10);
         _saveBtn.graphics.endFill();
         tf = new TextField();
         tf.selectable = false;
         tf.mouseEnabled = false;
         tf.defaultTextFormat = new TextFormat("_sans",12,16777215,true);
         tf.text = "SAVE CONTRACTS";
         tf.width = 140;
         tf.height = 26;
         tf.x = 0;
         tf.y = 4;
         ...
         _saveBtn.addEventListener(MouseEvent.CLICK,onSaveContracts);
         try
         {
            _root.stage.addChild(_saveBtn);
         }
         catch(e:Error)
         {
         }
      ...
      private static function onResize(e:Event) : void
      {
         ...
         if(_saveBtn)
         {
            _saveBtn.x = Math.max(0,_root.stage.stageWidth - _saveBtn.width - 10);
            _saveBtn.y = 40;
         }
      }
      ...
      private static function keepOnTop(e:Event) : void
      {
         ...
         try
         {
            ...
            if(_saveBtn && _saveBtn.parent)
            {
               _saveBtn.parent.setChildIndex(_saveBtn,_saveBtn.parent.numChildren - 1);
            }
         }
         catch(e2:Error)
         {
      ...
      private static function onSaveContracts(e:MouseEvent) : void
      {
         ContractLogger.saveLogs();
         info("STATE","Saving contract logs");
      }
