package com.oyunstudyosu.room
{
   ...
   public class RoomController
   {
      ...
      public function startload(param1:GameEvent = null) : void
      {
         ...
         try
         {
            ContractLogger.log("room_vars_read",{
               "roomKey":Sanalika.instance.roomModel.key,
               "width":Sanalika.instance.roomModel.width,
               "height":Sanalika.instance.roomModel.height,
               "title":Sanalika.instance.roomModel.title,
               "groupId":Sanalika.instance.roomModel.groupId
            });
         }
         catch(e5c:Error)
         {
         ...
         if(currentRoom.containsVariable("source"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"source",
                  "value":Sanalika.instance.roomModel.source
               });
            }
            catch(e5s:Error)
            {
         ...
         if(currentRoom.containsVariable("bots"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"bots",
                  "value":currentRoom.getVariable("bots").getStringValue()
               });
            }
            catch(e5b:Error)
            {
         ...
         if(currentRoom.containsVariable("doors"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"doors",
                  "value":currentRoom.getVariable("doors").getStringValue()
               });
            }
            catch(e5d:Error)
            {
         ...
         if(currentRoom.containsVariable("ownerID") && !currentRoom.containsVariable("isInteractiveRoom"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"ownerID",
                  "value":Sanalika.instance.roomModel.ownerId
               });
            }
            catch(e5o:Error)
            {
         ...
         if(currentRoom.containsVariable("ownerName"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"ownerName",
                  "value":Sanalika.instance.roomModel.ownerName
               });
            }
            catch(e5on:Error)
            {
         ...
         if(currentRoom.containsVariable("roomTitle"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"roomTitle",
                  "value":Sanalika.instance.roomModel.title
               });
            }
            catch(e5t:Error)
            {
         ...
         if(currentRoom.containsVariable("flatID"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"flatID",
                  "value":Sanalika.instance.roomModel.houseID
               });
            }
            catch(e5f:Error)
            {
         ...
         if(currentRoom.containsVariable("roomID"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"roomID",
                  "value":Sanalika.instance.roomModel.roomID
               });
            }
            catch(e5r:Error)
            {
         ...
         if(currentRoom.containsVariable("grid"))
         {
            ...
            try
            {
               ContractLogger.log("room_var_read",{
                  "name":"grid",
                  "length":currentRoom.getVariable("grid").getStringValue().length
               });
            }
            catch(e5g:Error)
            {
         ...
         try
         {
            ContractLogger.log("user_vars_read",{
               "user":sfs.mySelf ? sfs.mySelf.name : null,
               "gender":Sanalika.instance.avatarModel.gender,
               "clothes":_loc2_,
               "position":Sanalika.instance.avatarModel.position,
               "direction":Sanalika.instance.avatarModel.direction
            });
         }
         catch(e5u:Error)
         {
