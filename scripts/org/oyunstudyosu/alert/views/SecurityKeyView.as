package org.oyunstudyosu.alert.views
{
   ...
   public class SecurityKeyView extends CoreMovieClip implements IAlertView
   {
      ...
      public function SecurityKeyView()
      {
         super();
      }
      ...
      public function init() : void
      {
         ...
         Connectr.instance.gameModel.isSecurityKeyViewOpened = true;
         ...
         this.focusOut();
         if(this.vo.data.closeable != null && this.vo.data.closeable == false)
         {
      ...
      private function checkValidation() : void
      {
         ...
         if(_loc1_.length < 6)
         {
            ...
            this.errorVo.description = $("minimumSecretKeyLengthError");
         ...
         else if(_loc1_.length > 12)
         {
            this.errorVo = new AlertVo();
            this.errorVo.alertType = AlertType.TOOLTIP;
            this.errorVo.description = $("maximumSecretKeyLengthError");
            Dispatcher.dispatchEvent(new AlertEvent(this.errorVo));
         }
         else
         {
      ...
      {
         var _loc1_:String = this.inputManager != null ? this.inputManager.getText() : this.keyTxt.text;
         Connectr.instance.serviceModel.requestData(RequestDataKey.SECURITY_KEY,{
            "key":_loc1_,
            "id":this.sessionID
         },this.securityKeyResponse);
      }
      
      private function securityKeyResponse(param1:Object) : void
      {
         Connectr.instance.serviceModel.removeRequestData(RequestDataKey.SECURITY_KEY,this.securityKeyResponse);
         if(!param1.errorCode)
         {
            Connectr.instance.serviceModel.requestData(param1.sn,param1.requestData,null);
            dispatchEvent(new Event("closeClicked"));
         }
      }
      ...
      {
         Connectr.instance.gameModel.isSecurityKeyViewOpened = false;
